...
resource "google_bigquery_dataset" "ra_bq_dev_001" {
  ...
  access {
    role          = "OWNER"
    user_by_email = "ra-analytics-dev-474600@appspot.gserviceaccount.com"
  }
  access {
    role          = "READER"
...
resource "google_dataproc_cluster" "dataproc_cluster_dev" {
  ...
  cluster_config {
    ...
    gce_cluster_config {
      # Referência explícita à sub-rede no projeto Host
  ...
  depends_on = [
    ...
    # Adicionado para garantir que a sub-rede exista antes de tentar usá-la
